// frontend/src/utils/emailHtmlTransformer.ts
/**
 ...
 *   replaces dynamic tokens with rendered HTML, and ensures tables have legacy attributes.
 */

export interface TransformOptions {
  /** Replace {{inventory_table}} with actual rendered table HTML */
  inventoryTableHtml?: string;
  /** Base width for the email body container (default 600) */
  width?: number;
}
...
export function transformEmailHtml(
  ...
  // ─── 5. Replace inventory token with rendered table ───────────
  if (inventoryTableHtml) {
    html = html.replace(
      /<span[^>]*data-inventory-table-token[^>]*>[\s\S]*?<\/span>/gi,
      inventoryTableHtml
    );
    html = html.replace(
      /<div[^>]*data-token=["']inventory_table["'][^>]*>[\s\S]*?<\/div>/gi,
      inventoryTableHtml
    );
