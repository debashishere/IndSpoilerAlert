  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: 'hsl(var(--bg-card))', border: '1px solid hsl(var(--border-color))', borderRadius: '16px', width: '100%', maxWidth: '660px', overflow: 'hidden', boxShadow: '0 24px 60px rgba(0,0,0,0.6)' }}>
        ...
        <div style={{ background: 'hsl(223 47% 12%)', padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid hsl(var(--border-color))' }}>
        ...
        <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: '14px', maxHeight: '75vh', overflowY: 'auto' }}>
          ...
          {/* Metrics */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '9px', background: 'hsl(223 47% 8%)', padding: '13px', borderRadius: '10px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {stages.map((s, i) => (
                <div key={i} style={{ background: 'hsl(223 47% 9%)', border: '1px solid hsl(var(--border-color))', borderRadius: '9px', padding: '10px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: s.buyerMode === 'custom' && s.customBuyers && s.customBuyers.length > 0 ? '7px' : '0' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                      <span style={{ background: 'hsl(var(--primary))', color: '#fff', fontSize: '9px', fontWeight: 800, borderRadius: '3px', padding: '1px 6px' }}>Stage {s.stageIndex ?? i + 1}</span>
                  ...
                  {s.buyerMode === 'custom' && s.customBuyers && s.customBuyers.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                      {s.customBuyers.map((b: any) => (
                        <span key={b.id} style={{ fontSize: '10px', background: `${TIER_COLOR[b.tier || 1] || '#38bdf8'}18`, border: `1px solid ${TIER_COLOR[b.tier || 1] || '#38bdf8'}44`, color: '#e2e8f0', borderRadius: '10px', padding: '2px 8px' }}>{b.name}</span>
                      ))}
                    </div>
                  )}
          ...
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#38bdf8', fontWeight: 600, marginBottom: '8px' }}>Subject: {emailSubject}</div>
            <div
              style={{ background: '#0f172a', border: '1px solid hsl(var(--border-color))', borderRadius: '8px', padding: '12px', maxHeight: '140px', overflowY: 'auto' }}
        ...
        {/* Modal actions */}
        <div style={{ background: 'hsl(223 47% 10%)', padding: '14px 20px', display: 'flex', justifyContent: 'flex-end', gap: '10px', borderTop: '1px solid hsl(var(--border-color))' }}>
          ...
          <button type="button" onClick={onLaunch} disabled={isSubmitting} style={{ background: 'linear-gradient(135deg,hsl(var(--primary)),hsl(var(--secondary)))', border: 'none', color: '#fff', borderRadius: '8px', padding: '8px 20px', fontSize: '13px', fontWeight: 700, cursor: isSubmitting ? 'not-allowed' : 'pointer', opacity: isSubmitting ? 0.7 : 1 }}>
