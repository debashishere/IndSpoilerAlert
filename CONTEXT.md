...
- **Thread Bid Acceptance**: The action allowing a supplier to accept the latest incoming buyer proposal directly from the Negotiation History Thread, automatically transitioning the inspector to the `Accept Offer` tab within the same window while hydrating dynamic settlement tokens (`proposedPrice`, `proposedQuantity`, recalculated `total_amount`) to reflect the latest negotiated terms.
