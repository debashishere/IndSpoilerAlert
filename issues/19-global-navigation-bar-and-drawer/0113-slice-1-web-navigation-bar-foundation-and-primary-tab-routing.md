# 0113: Slice 1 - Web Navigation Bar Foundation & Primary Tab Routing
...
2. The center contains a rounded pill navigation container housing all primary module tabs: `Ingestion`, `Insight`, `Workflow`, `Marketplace`, `Inbox`, and `Settings`.
...
## Acceptance criteria
- [x] [index.html](file:///Users/debashisroy/Documents/SpoilerAlert/frontend/index.html) links `Hanken Grotesk` and `Material Symbols Outlined` with embedded SVG/Lucide fallback support.
- [x] `GlobalNavigationBar.tsx` is implemented under `frontend/src/components/navigation/` adhering to [web-nav-bar.html](file:///Users/debashisroy/Documents/SpoilerAlert/docs/stitch/web-nav-bar.html).
- [x] Active tab routing is synchronized with Redux `state.core.activeTab` and `setActiveTab`.
- [x] Desktop navigation tabs switch seamlessly across Ingestion, Insight, Workflow, Marketplace, Inbox, and Settings.
- [x] `GlobalNavigationBar.test.tsx` passes 100% with full coverage of rendering, tab clicks, and badge displays.
